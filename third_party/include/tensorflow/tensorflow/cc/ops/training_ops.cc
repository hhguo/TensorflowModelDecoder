...
ApplyDelayCompensatedGradientDescent::ApplyDelayCompensatedGradientDescent(const
                                                                           ::tensorflow::Scope&
                                                                           scope,
                                                                           ::tensorflow::Input
                                                                           var,
  ...
  if (!scope.ok()) return;
  auto _lambda = ::tensorflow::ops::AsNodeOut(scope, lambda);
  if (!scope.ok()) return;
  auto _shadow = ::tensorflow::ops::AsNodeOut(scope, shadow);
  if (!scope.ok()) return;
  ::tensorflow::Node* ret;
  const auto unique_name = scope.GetUniqueNameForOp("ApplyDelayCompensatedGradientDescent");
  auto builder = ::tensorflow::NodeBuilder(unique_name, "ApplyDelayCompensatedGradientDescent")
                     .Input(_var)
                     .Input(_alpha)
                     .Input(_delta)
                     .Input(_lambda)
...
ApplyDelayCompensatedGradientDescent::ApplyDelayCompensatedGradientDescent(const
                                                                           ...
                                                                           ::tensorflow::Input
                                                                           shadow)
  : ApplyDelayCompensatedGradientDescent(scope, var, alpha, delta, lambda, shadow, ApplyDelayCompensatedGradientDescent::Attrs()) {}
